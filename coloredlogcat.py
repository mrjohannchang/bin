...
HEADER_SIZE = DATE_WIDTH + 1 +TAGTYPE_WIDTH + 1 + TAG_WIDTH + 1 + PROCESS_WIDTH + 1
...
date_pattern = "\d\d-\d\d \d\d:\d\d:\d\d\.\d\d\d"
retag = re.compile("^(%s )?([A-Z])/([^\(]+)\(([^\)]+)\): (.*)$" % date_pattern)
...
while True:
    ...
    if not match is None:
        date, tagtype, tag, owner, message = match.groups()
        linebuf = StringIO.StringIO()

        if date == None :
            date = ""
        date = date.strip().ljust(DATE_WIDTH)
        linebuf.write("%s%s%s " % (format(fg=WHITE, bg=BLACK, bright=True), date, format(reset=True)))
